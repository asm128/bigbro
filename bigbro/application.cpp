...
static	::gpk::error_t										loadDBConfig				(::bro::SApplication & app)					{
	::gpk::view_const_string										jsonResult					= {};
	const ::gpk::SJSONReader										& configReader				= app.Framework.JSONConfig.Reader;
	const int32_t													indexObjectDatabases		= ::gpk::jsonExpressionResolve("application.bigbro.databases", configReader, 0, jsonResult);
	...
	for(uint32_t iDatabase = 0, countDatabases = (uint32_t)databaseArraySize; iDatabase < countDatabases; ++iDatabase) {
		...
		::bro::TKeyValJSONDB												& jsonDB						= app.Databases[iDatabase];
		...
		gwarn_if(errored(indexBindArray), "No bindings found for database file: %s.", dbfilename.begin());
