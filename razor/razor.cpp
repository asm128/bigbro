...
::gpk::error_t							processThisTable			(const ::gpk::view_const_string & missPath, const ::gpk::view_array<const ::bro::TKeyValJSONDB>	& databases, ::gpk::view_const_string & tableName)	{
	...
	for(uint32_t iTable = 0; iTable < databases.size(); ++iTable) {
		const ::bro::TKeyValJSONDB					& dbKeyVal					= databases[iTable];
		...
		for(uint32_t iTableMiss = 0; iTableMiss < cacheMisses.size(); ++iTableMiss) {
			...
			for(uint32_t iCacheMiss = 0; iCacheMiss < tableMiss.Val.size(); ++iCacheMiss) {
				...
				if(iBlockLast != iBlockCurrent) {
					...
					const ::bro::TKeyValJSONDB					& refDB						= databases[indexDB];
					::bro::TKeyValJSONDB						& newDB						= databases[databases.size() - 1];
